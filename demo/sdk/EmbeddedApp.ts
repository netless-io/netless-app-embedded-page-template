...
export class EmbeddedApp<TState = DefaultState, TMessage = unknown> {
  ...
  readonly onRoomMembersChanged = new EmbeddedPageEvent<
    DiffOne<ReadonlyArray<RoomMember>>
  >();
  ...
  connectStore<S>(storeId: string, ensureState?: S): Store<S> {
    ...
    if (!store) {
      ...
      store = new StoreImpl<S>({
        ...
        onSetState: (state) =>
          this._postMessage<"SetState", S>({
            NEAType: "SetState",
            payload: { storeId, state },
          }),
      }) as Store<S>;
  ...
  private _handleMsgStoreChanged(payload: unknown): void {
    if (Array.isArray(payload) && payload.length > 0) {
      const actions = payload as ToSDKMessagePayloads<
        TState,
        TMessage
      >["StoreChanged"];
