...
export interface ToSDKMessagePayloads<TState = DefaultState, TMagix = unknown> {
  ...
  StoreChanged: ReadonlyArray<
    AkkoObjectUpdatedProperty<{ [K: string]: TState }>
  >;
...
// iframe --> me
export interface FromSDKMessagePayloads<
  TState = DefaultState,
  TMagix = unknown
> {
  Init: void;
...
export type AddFromSDKMessageListener<TState = unknown, TMessage = unknown> = (
  listener: (
    message: FromSDKMessage<FromSDKMessageKey, TState, TMessage>
  ) => void,
