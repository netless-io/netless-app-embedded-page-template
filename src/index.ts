...
import { ensureAttributes, Logger } from "./utils";
  ...
  setup(context) {
    ...
    const safeListenPropsUpdated = <T>(
      ...
      const disposeReaction = context.mobxUtils.reaction(
        ...
        () => {
          ...
          if (isObj(props)) {
            disposeListenUpdated = () =>
              context.objectUtils.unlistenUpdated(props, callback);
    ...
    if (view) {
      ...
      if (room) {
        ...
        sideEffectManager.add(() => {
          ...
          return () =>
            displayer.callbacks.off("onRoomStateChanged", onRoomStateChanged);
        });
      }
    }
    ...
    const setState = (payload: unknown): void => {
      if (isObj(payload) && payload.storeId && isObj(payload.state)) {
        const { storeId, state } = payload as FromSDKMessage<
          "SetState",
          DefaultState
        >["payload"];
        if (!context.getIsWritable()) {
          logger.error(
            `Cannot setState on store ${storeId} without writable access`,
            state
          );
          return;
        }
    ...
    const setPage = (page: unknown): void => {
      if (!view) {
        logger.warn(
          "SetPage: page api is only available with 'scenePath' options enabled."
        );
      } else {
        ...
        if (
          ...
          if (
            room.scenePathType(fullScenePath) === ("none" as ScenePathType.None)
          ) {
    ...
    sideEffectManager.add(() => {
      ...
      const magixListener = (e: Event) => {
        if (
          e.event === magixEventChannel &&
          e.authorId !== displayer.observerId
        ) {
      ...
      return () =>
        displayer.removeMagixEventListener(magixEventChannel, magixListener);
    });
